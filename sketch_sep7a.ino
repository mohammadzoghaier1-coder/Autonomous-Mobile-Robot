...
// These two variables were missing from the code you sent.
// Put your original values here.
...
// TURN PID TUNING
const float TURN_SPEED_MAX = 180.0;
...
OutError OutputErrorForlaser()
{
  ...
  output = P_laser + I_laser + D_laser;
...
OutError OutputErrorForLeftWall()
{
  ...
  error = leftDistance - targetWallDistance;
...
OutError OutputErrorForRightWall()
{
  ...
  float output = 0;
  error = rightDistance - targetWallDistance;
  unsigned long currentTime = millis();
  float dt = (currentTime - distancePrevTime) / 1000.0;
  if (dt <= 0)
  {
  ...
  I_laser = constrain(I,-distance_INTEGRAL_LIMIT,distance_INTEGRAL_LIMIT);
...
void VLO_PID()
{
  ...
  // Reset encoder distance for this movement
  leftEncoderCount = 0;
  rightEncoderCount = 0;


  // Reset laser PID timing/error
  ...
  else if (leftDistance <= 15 && rightDistance >= 100)
  {
    while (TargetDistance())
    {
  ...
  else if (leftDistance >= 100 && rightDistance <= 15)
  {
    while (TargetDistance())
    {
...
void TurnRight90() {
  ...
  // Find new Direction
  LocalDirectionStates newDirection =
    (LocalDirectionStates)((CurrentDirection + 1) % 4);
...
void TurnLeft90() {
  ...
  LocalDirectionStates newDirection =
    (LocalDirectionStates)((CurrentDirection + 3) % 4);
...
{
  float ticksPerRev =
    encoderPolesCount * 2 * motorGearRatio;


  float wheelCircumference_cm =
    PI * wheelDiameter;


  long targetTicks =
    (long)((targetDistance_cm / wheelCircumference_cm) * ticksPerRev);


  long avgTicks =
    (leftEncoderCount + rightEncoderCount) / 2;
